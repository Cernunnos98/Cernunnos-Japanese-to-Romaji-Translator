...
- JSDoc/TypeScript `checkJs` with null-safety checking passes;
...
The complete release gate requires Node.js, Python 3, TypeScript (`tsc`), Chromium/Chrome and Graphviz `dot`. The runner accepts `CJ2R_PYTHON` (or `PYTHON`) for a non-standard Python 3 executable and `CHROMIUM` for a non-standard Chromium/Chrome executable.
...
| `run-translator-typecheck.js` | TypeScript `checkJs` + `strictNullChecks` validation |
