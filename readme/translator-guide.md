...
JMnedict translations for works, products, companies and organisations establish named-entity identity; they do not by themselves establish Rule-0 source spelling. A translated, localised or original-language work title that does not represent the Japanese surface must not be promoted as that surface's direct Romaji output. Direct source spelling requires independent evidence that the Roman form corresponds to the Japanese name itself; otherwise the surface remains review-only or is handled by a more narrowly scoped evidence bank. When the Japanese surface itself is a foreign-derived title, CJ2R follows the source words actually encoded by that surface rather than substituting a different original or localised work title. For example, a Japanese release title equivalent to `ザダイバー` may establish `The Diver`; the fact that the same work is `Men of Honor` does not make `Men of Honor` the source spelling of `ザダイバー`.
