...
Final-pass diagnostics that detect a local orthographic or lexical condition must likewise attach the exact local source span. They must not fall back to whole-input ownership merely because the condition was detected after token resolution; repeated occurrences remain separate review signals with separate source ranges. Whole-input ownership is reserved for genuinely output-wide guards.
...
U+303E `〾` IDEOGRAPHIC VARIATION INDICATOR is also an unresolved-orthography condition. Unicode defines the following ideograph as an approximation of an intended character rather than the intended character itself. The indicator therefore emits `[Unresolved]` and keeps `requiresReview`; the following approximate ideograph may remain visible/transliterated as diagnostic evidence, but it cannot make the complete result authoritative. U+303F `〿` is a separate visible display-cell filler and is not covered by this rule.

Assigned Hiragana/Katakana characters in Unicode Kana Supplement (U+1B000–U+1B0FF) and Kana Extended-A (U+1B100–U+1B12F) are likewise inside Japanese orthographic scope because Unicode defines those blocks for historic/variant Japanese kana and hentaigana. In explicit historical mode, the 274 hentaigana code points for which the Unicode/CODH/NINJAL correspondence is single-valued are structurally normalised to their maintained modern-hiragana value before tokenisation. Eleven explicitly multi-valued hentaigana are never given a default value. In historical mode, CJ2R may resolve one only when the complete contiguous kana run contains surrounding kana context and exactly one candidate normalisation has **strong maintained whole-word lexical reading evidence** in the existing kana-lexical index. Zero matches, multiple matches, standalone glyphs, and candidate explosions remain unchanged and review-required; no frequency or tokenizer probability is used. U+1B001 is handled as a separate dual-identity case: Unicode names it archaic *ye* while also aliasing it as hentaigana *e*. Historical mode therefore retains the bounded candidates `え` and Rule-0 `いぇ` and applies the same complete-run, unique-strong-lexical-evidence rule; it has no standalone default. All assigned characters in those two historical kana blocks, plus the nine Unicode 17 historic Small Kana Extension characters U+1B132, U+1B150–U+1B152, U+1B155 and U+1B164–U+1B167, remain inside the contextual run boundary even when unresolved, so an unresolved extension cannot split a longer run and allow a fragmentary lexical rescue. The production resolver evaluates the eleven multi-valued hentaigana and U+1B001 together across the complete run. Normal mode never applies this conversion. Other characters in these blocks remain `[Unresolved]` without maintained reading evidence. Katakana Phonetic Extensions are deliberately not swept into this rule because their primary orthographic use is materially distinct. When a single-valued hentaigana carries dakuten or handakuten, historical normalisation first maps the base glyph to ordinary hiragana and only then applies the existing kana voicing/semivoicing composition rules; NINJAL's glyph database explicitly records both `濁点` and `半濁点` on hentaigana. Diacritics do not resolve a multi-valued base. Historical mode also structurally expands Unicode's explicitly equivalent historic forms `𛀀`, `𛄣`–`𛄨` to their maintained ordinary-kana equivalents before tokenisation. Unicode assigns the fixed syllabic identities `wu`, `yi`, `ye`, `wu` to `𛄟`, `𛄠`, `𛄡`, `𛄢`. CJ2R normalises those four only in historical mode to the already-supported Rule-0 kana spellings `うぅ`, `いぃ`, `いぇ`, `うぅ`, which the ordinary Hepburn/Wāpuro core romanises as `wu`, `yi`, `ye`, `wu`. This is an encoded-syllable transliteration rule, not a claim that all four contrasts are independently attested in historical Japanese speech.

Unicode L2/16-354 specifically documents small WI/WE as historic labialisation forms and attests `くゐ`/`ぐゐ` and `くゑ`/`ぐゑ`; historical mode therefore reduces only k/g + encoded small WI/WE to the existing `kwi/gwi/kwe/gwe` Rule-0 path. Small WO, small katakana N and small KO remain unresolved: the available evidence does not establish one safe Japanese historical reading rule for them, and small N is explicitly documented as a nasalisation form. The Small Kana Extension block is never normalised wholesale.
...
Reviewed name evidence establishes a valid name reading; it does not prove that every occurrence of the same spelling is a name. When an exact single Kuromoji token is analysed as an ordinary lexical noun with a conflicting reading, preserve the reviewed name output as a candidate but retain `reading-evidence-conflict` review unless independent name-role evidence settles the occurrence. A directly attached reviewed honorific is sufficient name-role evidence, so role-compatible person-name candidates outrank an ordinary lexical or location interpretation for that occurrence; however, the honorific does not choose between multiple maintained person-name readings, and genuine name-reading ambiguity remains review-required. When that role is established but several maintained person-name readings remain tied, the highest-ranked maintained person candidate may be emitted only as a provisional fallback with review retained; this does not promote it to a resolved reading. If analyser tokenisation swallows a boundary that can instead be parsed as a maintained person name plus a recognised honorific, preserve the analyser output provisionally but keep the competing name/honorific boundary review-required rather than forcing either structure. Do not silently erase an ordinary-word homograph merely because the same surface has a reviewed surname or place reading.
...
Reviewed name spans may detach recognised honorific suffixes such as `様`, `君` and `氏`. Suffix readings remain independently evidence-backed. Final output boundary arbitration must also retain reviewed-name provenance: a following Kuromoji `接尾/一般` lexical unit is separated and capitalised as its own word rather than being blindly glued to canonical name output. Structural location/facility suffixes keep their existing independently adjudicated morphology and review rules.
...
### Explicit common-word token-reading authority

A reviewed common-word object may set `tokenReadingAuthority: true` only when independent evidence establishes that its pronunciation remains authoritative at an edge of a larger compositional Kuromoji token. The repair is reading-preserving rather than boundary-destructive: CJ2R may replace an independently attested analyser reading for that component while retaining the analyser reading of the untouched remainder of the token.

This is an explicit opt-in. Ordinary common-word evidence cannot rewrite a substring of an intact token. The mechanism requires exact Kuromoji dictionary evidence for the component reading being replaced, does not apply inside proper-name tokens, and must be protected by compound and inverse regression controls.
...
Reviewed loanword context is relation-scoped rather than bag-of-words. The final verification pass may use evidence from a direct compound/genitive relation or from the predicate governing the loanword's case-marked phrase. A cue that merely appears in a neighbouring argument does not settle donor ambiguity. Raw source-text matching is reserved for reviewed phrases that include the loanword surface itself; this prevents a shorter cue such as `カー` from matching incidentally inside an unrelated word such as `カーテン`.

JMnedict named-entity translations are **identity evidence, not automatically source-spelling evidence**. A translated, localised or original-language title can identify a work without spelling the Japanese title itself. Before a JMnedict-derived work/product/organisation label is promoted to global direct Romaji, independently verify that the Roman output corresponds to the Japanese surface under Rule 0. If that cannot be established, keep the surface scoped or review-only; do not import a translation alias as source spelling. For a foreign-derived Japanese release title, preserve the source words actually encoded by the Japanese surface even when the same work has a materially different original or localised title; work identity is not a licence to replace the surface.
...
- **Japanese compatibility source symbols:** M05 may apply the explicit Unicode compatibility decomposition to characters in U+3200–U+33FF and U+1F200–U+1F2FF only when that decomposition contains Japanese script and consists solely of the maintained safe source inventory. This covers Japanese parenthesised/circled/squared forms, era/corporation symbols, Katakana unit/source forms and Japanese ARIB-style enclosed symbols without turning P07 into a global NFKC pass. Compatibility symbols whose decomposition is non-Japanese, such as squared Latin measurement notation, remain outside this rule. Any eligible Japanese compatibility symbol that survives to final output is blocked as unresolved rather than silently trusted.
- **References:** Unicode Standard Chapter 22 compatibility-decomposition model; Unicode CJK Compatibility and Enclosed CJK Letters and Months charts; Unicode Enclosed Ideographic Supplement chart.
...
- **Rules / constraints:** Retokenise only hard-boundary segments. Preserve complete-sentence analysis elsewhere and retain positions/boundary semantics. `×` is a canonical hard boundary so Kuromoji's possible `カケル` reading cannot become authoritative merely because the symbol was tokenised as a word; reviewed title evidence may subsequently reinterpret the source span. ASCII `~` remains a true segment boundary and must not be swallowed by Latin passthrough. A Japanese censorship marker is opaque source material: it blocks inference across the hidden span. When that isolation causes the immediately following visible Han token to be analysed as a proper name but one unique maintained ordinary lexical reading conflicts with that isolated role, retain the ordinary visible reading and the conflict as review-required evidence rather than promoting the name reading. If the ordinary lexical evidence itself has multiple readings, do not invent a new selection.
...
- **Rules / constraints:** Distinguish calendar-date `一日`/`1日` from duration/frequency use using structural context; kanji, ASCII-digit and supported full-width numeric source forms must enter the same role arbitration. Predicate detection includes ordinary verbs/adjectives and morphological `サ変接続` noun + `する` structures, including inflected `する` and an explicit `を`; it may also see through an intervening case-marked nominal phrase to the eventual clause predicate. Strong duration structure also includes the `で` time-span frame and established amount/range/distributive followers such as `分`, `間`, `以内`, `以下`, `以上`, `未満`, `程度` and `ごと`. Counted-day/offset morphology such as `一日目`, `一日後`, `一日前`, `一日ぶり` and duration `一日半` independently establishes the non-calendar `いちにち` role when no explicit calendar predecessor owns the span. An explicit calendar-month predecessor, including a numeric month expression, `毎月` or `各月`, establishes the calendar-date role before ordinary date-relative continuation; for example, `4月1日後` remains based on `ついたち`. A following `に` normally remains compatible with the calendar reading, but `に` immediately followed by a recognised frequency count such as numeral+`回`/`度` is positive duration/frequency structure and selects `いちにち`. The following count is then marked as a selected `frequency-counter`: reviewed whole-counter evidence remains authoritative where available; otherwise a regular source-digit count may preserve the written numeral plus an independently established counter-unit reading. If no independent role evidence resolves `calendar-date` versus `duration-day`, retain the provisional calendar reading with active review state; lexical continuation such as `限定` and punctuation are not resolving evidence. Do not globally replace the legitimate `ついたち` reading.
- **Uses:** `data/grammar/counter-date-reading-evidence.json`
- **Focused QA:** `numbers-and-counters.js`

**T06b — Explicit day-duration span readings**

- **Owner:** M09 `markTypedDayDurationSpanTokens`
- **Flow:** T06 tokens → explicit `[day-count]日間` duration spans
- **Rules / constraints:** An explicit `日間` span establishes a duration role independently of calendar-date context. For day counts whose conventional duration reading is irregular, reuse reviewed `日` evidence after the role has been established: `一日間 → いちにちかん`, `二日間 → ふつかかん`, `三日間 → みっかかん`, and the reviewed irregular classes such as `十四日間`, `二十日間` and `二十四日間`. ASCII/full-width numeric aliases may use the same reviewed evidence. The repair must reconstruct a reviewed irregular span across analyser splits such as `十 + 四日間`, but must not reinterpret a nested suffix inside a larger regular numeral: `十一日間`, `十二日間` and other regular forms remain on the existing regular `にち` path, and regular source digits retain the established digit-style rendering. No reading is inferred merely because a numeral precedes `日`; the irregular repair requires an explicit `日間` duration structure plus reviewed day-reading evidence.
- **Uses:** `data/grammar/counter-date-reading-evidence.json` after the duration role is structurally selected
- **Focused QA:** `numbers-and-counters.js` (`MECH-NUMERIC-ROLE-DAY-DURATION-*`).
...
- **Rules / constraints:** Role selection precedes pronunciation. Reviewed counter/date rows carry an explicit `role` (and `unit` where applicable); their pronunciation must not be used as evidence that the source span has that role. `clock-hour` is selected only for a structurally valid numeral+`時` span with reviewed clock evidence, excluding `時間`. `minute-counter` is selected only for a structurally valid numeral+`分` span when no strong incompatible lexical interpretation remains unresolved. Same-span lexical/counter collisions such as standalone `十分`/`十二分` remain reviewable unless an independently reviewed contextual lexical candidate owns the exact span; for example, `十分に`/`十二分に` may select their lexical `ぶん` readings while contextless `十分` and genuinely ambiguous `十分待つ` remain review-required. A following `間` establishes a duration only when that boundary is not crossed by stronger whole-lexeme evidence: an analyser `分間` split may be repaired to preserve an independently attested continuation such as `間違う`, so `十分間違う` cannot be forced into a ten-minute duration merely because `分` and `間` are adjacent. Contextless terminal `一日` likewise records an unresolved `calendar-date` versus `duration-day` temporal role; CJ2R may expose a provisional date reading but must retain an active `temporal-role-ambiguous` signal until actual context resolves the role. Punctuation is not resolving evidence. Transparent quotation/bracket/parenthesis boundary tokens likewise do not create a numeric role, but they must not hide independently sufficient neighbouring structure already recognised by the same role classifier (for example 午前「一時」, 4月「1日」 or 三時「十分」). Hard punctuation remains blocking. A context-scoped reviewed common-word candidate followed morphologically by `する` may likewise retain the lexical head for the later common-word merger (for example `三分する` and `四分する`); none of these cases generalises `ぶん` to arbitrary numeral+`分` forms.
...
- **Rules / constraints:** Pronunciation runs only after a compatible role has been selected. Reviewed exceptions are preferred for their declared role; productive minute morphology is scoped only to an already-selected `minute-counter` span. The minute rules cover the supported Japanese minute class (`1/6/8/10` sokuon + `ぷん`, `3/4` + `ぷん`, otherwise `ふん`) and are not a universal counter gemination/voicing heuristic. Reviewed `numeric-component` evidence is composed before the counter allomorph is selected, so irregular numeral components such as `三百`, `六百`, `八百`, `三千` and `八千` retain their established readings inside larger Kanji counts instead of being re-read mechanically from individual characters. The duration suffix `間` is then attached after the selected minute reading, preserving forms such as `二十一分間 → Nijuuippunkan`; when a larger minute count contains completed Rule-0 numeric groups, only the final lower-value group is composed with the minute counter and the earlier groups remain space-separated (for example `三百一分間 → Sanbyaku Ippunkan`). The separately attested `百分` boundary uses `Hyappun`, while larger-unit forms outside that documented 百 contraction remain on their independently supported path. Counter families such as `本`, `匹`, `杯`, `発`, dates and month/place counters continue to use reviewed class-specific evidence rather than sharing this rule. In a selected regular frequency-counter frame, an unreviewed Arabic numeral may preserve its source digits with the independently established unit reading (for example `3回 → 3kai`); reviewed irregular whole-counter evidence still outranks that regular fallback. Reviewed Kanji counter rows may additionally declare `numericTail: true` when their attested counter allomorph is valid as the final tail of a larger Kanji count or 第-prefixed ordinal; the engine composes only the final Rule-0 numeric group with that reviewed tail (for example `十一回 → Juuikkai`, `第十一回 → Daijuuikkai`) and preserves spaces after completed higher-value groups (for example `三百一回 → Sanbyaku Ikkai`). A reviewed `百+counter` row may separately declare `hundredTailReading` when that counter has an attested sound change specifically at a numeral ending in 百; the engine preserves the already-resolved hundred numeral (`百`, `三百`, `六百`, `八百`, etc.) and replaces only its final `く` boundary with that reviewed counter tail. This permission is counter-family evidence, not a universal 百 contraction rule, so unaffected families such as `冊`, `歳`, `章`, `節`, `人` and `話` remain on their ordinary `hyaku-` path. Neither contract applies to digit-preserving larger forms, and reusable tails must not be inferred from suppletive rows such as `一人`/`二人` or the exceptional whole form `二十歳`. Adjacent typed `clock-hour` and `minute-counter` components remain separate output units.
...
- **Rules / constraints:** Execute only when historical mode was requested. This whole-word historical evidence pass remains separate from the narrow modern-mode obsolete W-row normalisation described above. Exact evidence may cross kana iteration-mark symbol tokens (`ゝ/ゞ/ヽ/ヾ`) when the complete source span is maintained; the mark itself is never treated as permission for a general historical rewrite.
- **Uses:** `data/historical-kana/historical-kana-evidence.json`
- **Focused QA:** `historical-kana.js`

**Pre-tokenisation historical vertical iteration normalisation**

- **Owner:** M05 `normalizeHistoricalVerticalIterationMarks`, invoked by M10 `translateText` only when historical mode is requested.
- **Flow:** ordinary input normalisation → historical vertical iteration normalisation → Kuromoji.
- **Rules / constraints:** This is structural symbol handling, not a lexical historical-spelling heuristic. U+3031 `〱` repeats the immediately preceding kana pair; U+3032 `〲` repeats that pair with voicing on the first repeated kana. U+3033 `〳` + U+3035 `〵` and U+3034 `〴` + U+3035 `〵` are canonicalised to their complete unvoiced/voiced repeat forms before expansion. U+303B `〻` is canonicalised to the existing U+3005 `々` path. Malformed, incomplete, or unvoiceable forms are left intact so the unresolved-orthography guard can require review.
- **Isolation:** normal mode does not apply these historical vertical-mark semantics. Existing exact historical lexical evidence remains exact and continues to outrank structural fallback only where separately attested.
- **References:** Unicode Standard CJK Symbols and Punctuation (U+3031–U+3035, U+303B); Unicode Core Specification description of the vertical kana repeat marks; Japanese dictionary references for `踊り字` / `くの字点`.
- **Focused QA:** `historical-kana.js`
...
- **Ideographic decimal notation:** U+3007 `〇` is treated as a positional decimal zero only inside tokenizer-confirmed numeric material. A simple decimal digit run containing `〇` may be canonicalised to ASCII digits for output (`二〇二六 → 2026`), but `零`, multiplicative numeral units such as `十/百/千`, and analyser-classified symbolic `〇` are not rewritten. Role-specific numeric evidence is applied first, so `〇時` reuses the reviewed `0時 → れいじ` clock reading rather than becoming `0ji`.
- **Uses:** `data/grammar/counter-date-reading-evidence.json`
- **Focused QA:** `numbers-and-counters.js`
...
- **Rules / constraints:** Preserve candidate/ambiguity and source-coverage metadata. `mergeSafe` answers only the span-boundary question; it does not prove reading uniqueness. Jitendex/Yomitan scores are popularity/search-order metadata, not semantic confidence. General-word evidence is fallback/rescue, not authority to overwrite stronger reviewed mechanisms or an already-tokenised numeral + 助数詞 structure merely because the combined orthography also exists in the general-word bank. Hard punctuation may delimit an exact ambiguous lexical segment. Transparent wrappers such as quotation marks or parentheses may expose the same isolated segment for fallback, but the wrapper itself is never reading evidence: it must preserve the bare segment's provisional reading/review state, and must not suppress genuine neighbouring grammatical or contextual evidence outside the wrapper.
...
- **Rules / constraints:** Assess conflicts against reading/general/name evidence. Kuromoji is important context, not absolute authority. A Kuromoji-selected reading plus unrelated neighbouring text is not sufficient contextual evidence to clear maintained alternatives; weak ambiguity is superseded only by a recognised contextual resolver or an independently established structural/grammatical role.
...
- **Owner:** M10 `classifyTokenStructuralBoundaryDecision` + `needsCrossTokenApostrophe`
...
- Explicit proper-noun analyser role may support a maintained provisional proper-name candidate fallback for ambiguous multi-kanji names; ambiguity remains review-required and lexical tokens are excluded.
